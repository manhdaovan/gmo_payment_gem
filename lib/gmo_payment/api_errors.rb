module GmoPayment
  class ApiErrors
    ERRORS        = {
      ...
      # 本サービス決済センター・フロント側アプリケーションにて検知されたエラー
      # カード決済以外の決済手段での発生エラー(入力パラメータエラー等)
      # ※一部のエラーはカード決済時にも発生します。
      ...
      # Pay-easy、コンビニ（セブンイレブン以外） センターでのエラー
      # (通信エラーなど)
      'W0100W001' => 'データ種別不正',
      ...
      # フレッツまとめて支払い決済返却エラーコード
      # ※：決済事業者によって、下記に記載されているコード以外が返却される場合もあります。
      'FL1001001' => '会員認証中断',
